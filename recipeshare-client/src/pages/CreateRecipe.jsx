...
export default function CreateRecipe({ mode='create' }){
  ...
  return (
    <form onSubmit={handleSubmit} className="row" style={{ flexDirection:'column', gap:12, maxWidth:720, margin:'0 auto' }}>
      ...
      <input className="input" placeholder="Title" value={recipe.title} onChange={e=>setRecipe(s=>({...s,title:e.target.value}))} />
      <textarea rows={3} className="input" placeholder="Short description" value={recipe.description} onChange={e=>setRecipe(s=>({...s,description:e.target.value}))}></textarea>
      <div className="row" style={{ gap:12 }}>
        <textarea rows={8} className="input" placeholder={'Ingredients (one per line)'} value={recipe.ingredients.join('\n')} onChange={e=>setRecipe(s=>({...s,ingredients:e.target.value.split('\n').map(t=>t.trim()).filter(Boolean)}))}></textarea>
        <textarea rows={8} className="input" placeholder={'Instructions (one step per line)'} value={recipe.instructions.join('\n')} onChange={e=>setRecipe(s=>({...s,instructions:e.target.value.split('\n').map(t=>t.trim()).filter(Boolean)}))}></textarea>
      </div>
      <input className="input" placeholder="Categories (comma separated)" value={recipe.categories.join(',')} onChange={e=>setRecipe(s=>({...s,categories:e.target.value.split(',').map(t=>t.trim()).filter(Boolean)}))} />
      <div className="row">
